...
## 2026-08-22 03:40 — Discordセッション

- 症状：「IonoMonitorの通知が届かない」との報告 → 調査（コード変更なし）
- 調査内容：
  - `?action=test-push` で全購読者へ診断送信 → 31/31件成功（410なし）。サーバー側（Worker・VAPID暗号化・送信処理）は正常と確認
  - `webpush_state` のクールダウン（2時間）を確認 → 当該ユーザーの端末以外は正常に動作中と判断
- 原因判明：ユーザーがSafariの通常タブから通知登録していたため、ホーム画面アイコン（スタンドアロンPWA）側の購読が未登録状態だった
  - iOSのWeb Pushは「ホーム画面に追加したアプリとして開いた状態」で登録しないと、そのアプリ単位の通知許可（設定 > 通知 の一覧）に反映されない
- 対応：ホーム画面アイコンから開き直し、通知ボタンを再度タップして登録 → 購読数31→32件に増加、テスト通知の受信を確認
- 教訓：今後同様の「通知が届かない」報告があれば、まず「ホーム画面アイコンから開いて登録したか」を確認する
