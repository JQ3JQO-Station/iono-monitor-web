# DEVLOG — CB DX Iono Monitor

セッション間の作業引き継ぎログ。Claude が各セッション終了時に追記する。

---

## 2026-08-07 08:xx — Desktop セッション

- GitHub Actionsから「Update FxEs History workflow run」失敗（cancelled）のメール通知を受け、原因調査を実施
- 原因: コード側のバグではなく、GitHub Actions ホストランナー側の一時的な障害
  - `gh run view` のannotationsに `The job was not acquired by Runner of type hosted even after multiple attempts` と記録
  - 同時間帯（2026-08-06 15:53〜18:04 UTC ≒ 8/7 00:53〜03:04 JST）に `fetch-data.yml` も2回連続失敗しており、単発のワークフロー不具合ではなくGitHub Actions全体の一時的な障害と判断
  - この間 `docs/data.json` が約10時間更新されていなかった（最終成功: 8/6 13:23 UTC）
- 対応: `gh workflow run fetch-data.yml` / `gh workflow run update-fxes-history.yml` で手動再実行 → 両方成功、データ復旧済み（コード変更なし）
- 併せて、同時間帯にdisordeBotのDiscordブリッジも無応答だった件を調査。原因はDiscordブリッジ(server.ts)がClaude Desktopアプリ起動中のみ動作する仕様のため、アプリ未起動中に届いたメッセージに応答できなかっただけ（インフラ障害とは無関係、コード修正不要）
- 再発した場合は `gh run list --workflow=<name>.yml` で failure/cancelled が続いていないか確認し、GitHub側の障害なら手動再実行で復旧可能

---

## 2026-05-31 10:17 — Discord セッション（新規プロジェクト：CBノイズ軽減ユニット）

- 新規ハードウェアプロジェクト：CBラジオ用パッシブノイズ軽減ユニットの設計
- 要件定義書（CB_noise_unit_requirements_v1.md）をもとに壁打ち設計
- 確定設計：LPF2段（A/B）パッシブシャントフィルター
  - VR1＋10Ω並列：LPF-A深さ調整（実効0.9〜9Ω）
  - VR2＋10Ω並列：LPF-B深さ調整
  - ロータリーSW1：LPF-A折れ点（2.2/4.7/10μF）
  - ロータリーSW2：LPF-B折れ点（4.7/10/22μF）
  - C_DC 100μF NP：入力DCカット（汎用機対応）
  - トグルSW ON-ON：全バイパス（信号ルーティング方式）
- 配線手順書を /home/shigeki/projects/CB_noise_filter_wiring_v1.md に保存
- 追加調達部品なし（全部品手持ちで揃っている）

---

## 2026-05-28 22:30 — Discord セッション

- monitor.html: FxEs黄色閾値を 7.0 → **6.0** に変更（18MHz開通圏を反映）
- worker/index.js: Webプッシュ通知トリガー閾値を 7.0 → **6.0** に変更
- 通知メッセージ文・monitor.html登録モーダル説明文も 7.0→6.0 に更新
- Workerデプロイ済み、GitHub push済み

---

## 2026-04-13 15:46 — Discord セッション

- FxEsマーカーを四角形に変更
- 直近4回分（1時間）の履歴をマーカー内に表示（過去3回小さく上部、最新値大きく下部）
- 過去3回分でEs発生（>= 9.0）の値を赤字太字に変更
- 沖縄マーカー位置を微調整（右に1/4移動）
- **Cloudflare Worker に Cron 追加（最重要）**
  - `worker/index.js` 新規作成 → デプロイ済み
  - 15分ごと自動実行でNICT直接取得 → 東京・鹿児島 FxEs >= 9.0 でLINE送信
  - KV(IONO_STATE) で重複防止（TTL 2時間）
  - ブラウザ不要でLINEアラート送信可能になった

---

## 2026-04-14 — Desktop セッション

- corsproxy.io が403になったため、自前Cloudflare Worker（GET ?url=）に切り替え
- スポラジック → スポラディック スペル修正
- FxEs赤マーカー時の矢印を白色に変更
- LINEアラートの重複送信問題を解決
  - 原因: Discord セッションで実装済みのCron Workerに加え、ブラウザ側でも独立してLINE送信していた
  - 修正: ブラウザ側のLINE送信コードを削除、バナー表示のみに
  - LINE送信はCron Worker（KV管理）に一本化
- CLAUDE.md / DEVLOG.md 新規作成（セッション間引き継ぎ体制を整備）

---

## 2026-05-25 — Discord セッション

### Es レーダー テストページ作成（`docs/es-radar.html`）
- どこにもリンクしない非公開テストページとして新規作成
- 実装内容：
  - 日本地図SVGに雨粒型（Teardrop）マーカーを重ねて表示
  - マーカーの大きさ = FxEs値に比例、色 = 強度（シアン→緑→黄→橙→赤）
  - 方向 = 全局の加重平均緯度（Es重心）に向かって丸い側が向く
  - ステータスサマリーカード（「Es発生中: 鹿児島 6.8MHz / 東京 6.6MHz」等）
  - 地点間を繋ぐぼかし帯（鹿児島↔東京・東京↔北海道）
  - 白背景（monitor.htmlと同じデザイン）
  - 5分ごと自動更新

### 課題・未解決
- **地点間の表現が直感的でない**：4局の離散データから地点間状況を「見せる」ことの難しさ
  - 現状：ぼかし帯でつないでいるが「ぼんやりしているだけでわかりづらい」（karamasu0134 評）
  - 根本問題：地点間のデータは存在せず、表示はすべて推定
  - 次回検討：「回線ごとの伝播可否」表示、観測値と推定範囲の明示分離など

---

## 2026-05-25 続き — Discord セッション

### es-demo.html 大幅改善（`docs/es-demo.html`）
- **扇形（セクター）表示**を実装：地理方位ベースのスキップゾーン扇形でチャンス方向を表示
  - 線による接続を廃止、エリアで伝播チャンスを示す方式に変更
  - geoBearing()で大阪から各局への地理方位を計算、SVG角度に変換
  - 扇形エッジを `feGaussianBlur` でぼかし（気象レーダー風）
  - FxEs≥9のとき二重扇形（ぼかし+シャープオーバーレイ）＋パルスアニメーション
- **地図全幅化**、**気象レーダー風雲エフェクト**追加
- **デザインポリッシュ**：大阪クロスヘアマーカー、局ラベル白フチ、カラー調整
- **ナレーションボックス削除**（karamasu0134 判断：老眼で読めない・不要）

### 設計議論・検証（未実装）

#### foEs vs FxEs の整理
- FxEs は常に foEs より 0.5〜1.5MHz 高い「見かけ値」
- **方向検出（相対的強弱）→ FxEsで十分**
- **閾値判定（R27）→ foEsが必要だが、実際の運用では理論値通りにならないため FxEs で実用上は問題なし**

#### Es移動方向検出の検証結果（2026-05-24データ）
5/24 10:00〜12:00のFxEs加重平均緯度（重心緯度）の推移：

| 時刻 | 重心緯度 | 変化 | 意味 |
|------|---------|------|------|
| 10:00-10:30 | 35.1-35.2°N | 安定 | 関東Es（東京中心） |
| 10:45 | 33.6°N | **急落-1.6°** | 南西移動 or 別Es発生 |
| 11:00-11:15 | 33.5-33.8°N | 安定 | 九州付近で定在 |
| 11:30-11:45 | 34.3-34.5°N | **緩やかに北上** | 北東への拡大 |
| 12:00 | 34.5°N | 安定 | 鹿児島ピーク・北海道上昇 |

**検証まとめ：**
- **1ステップ（15分）での急激ジャンプ（>1°）** → 移動か新規Esセル発生か判別困難
- **連続2ステップ以上で同一方向に重心が動く** → Es移動と見なせる
- 重心緯度の時系列変化が「移動検出」に使えることが確認できた

#### 実装方針（次回セッションで実装）
- アルゴリズム：前2ステップの重心緯度の変化方向を計算
  - `Δ重心 > +0.3°/ステップ が 2連続` → 北上中
  - `Δ重心 < -0.3°/ステップ が 2連続` → 南下中
  - 単発ジャンプ → 「急激な変化（移動 or 新発生）」として扱う
- 表示：現状の扇形に「予測扇（薄色・点線）」を移動方向に追加
  - 北上中 → 北東方向に薄い予測扇
  - 南下中 → 南西方向に薄い予測扇
- デモ（HISTORY）で動作確認してから本番に組み込む
- foEsデータ取得は現時点では保留（FxEsで代替）

---

## 2026-05-26 — Discord セッション

### es-demo.html 実装（Bb + 重心マーカー）
- **アニメ風雲シルエット**に全面差し替え（blurなし、白輪郭あり、はっきり見える雲形SVGパス）
  - FxEs強度に応じてscale(0.4〜1.6)・opacity変化（CSS transition 0.9s）
  - 各局固定位置に雲を配置、ステップ変化で雲が大きくなる/小さくなる
- **重心マーカー**：オレンジ太字十字（#ff8800、stroke黒）
  - 急ジャンプ（>35 SVG units）→ テレポート＋centroid-spawnフラッシュアニメ
  - 緩やかな変化 → CSS transitionでスムーズに移動
  - 白背景で視認できるよう色変更済み

### 設計議論・方向性整理

#### 重心マーカーの問題
- 「ここがEsの中心です」に見えてしまい誤解を招く（karamasu0134指摘）
- 交信エリアを示すものではなくあくまで演出
- 現状の「強さ表示」だけでは移動方向が読めないという根本課題が残っている

#### 論文・記事の参照
- **北大論文（Earth Planets Space 2024）**：EsはE-W方向に最大180km・幅10〜20kmの細長い帯状、移動速度~90m/s（15分で~80km）。イオノゾンデは点観測なので詳細な空間構造は不明。
- **掲載記事（ameblo JL7KHN）**：R27 = foEs/27MHz で開通判定。4局のFxEs変化率比較でEs南北移動・先行開通エリアを特定する設計。foEs/h'Es/fbEsの3指標を活用。

#### 次の開発方向（確定）
「今どこが開いているか」＋「次にどこが開きそうか」を示す。

1. 4局のFxEs変化率を比較してEs移動方向を判定（北上中/南下中/拡大中/縮小中）
2. 移動方向インジケータ（矢印など）を地図に追加
3. 上昇率が高い局の方向に「予測扇（薄色・点線）」を表示

**重要な制約**：現在のデータはFxEsのみ（foEs/h'Es/fbEsなし）。予測はあくまで参考値として表示すること。

**次回の作業**：表示デザインを先にラフスケッチで合意してから実装に入る（訳のわからない表示にならないよう慎重に）。

---

## 2026-05-26 続き — Discord セッション

### es-demo.html 追加実装
- **現在/予測タブ切替**を実装
  - 現在タブ：今まで通りの扇形＋雲表示
  - 予測タブ：前ステップより0.3MHz以上上昇した局方向に青い点線扇のみ表示（雲・重心マーカーは非表示）
- **ライブモードを追加**（右上「● ライブ」ボタン）
  - data.jsonから現在値＋前回値（ok_prev等）を取得してリアルタイム表示
  - 5分ごと自動更新
  - 「デモへ戻る」でデモ再生に切り替え可能

### 方向性の再整理（重要）

karamasu0134との議論を経て、プロジェクト方針を明確化：

**「いつ・どこが上がるか」（事前予測）は断念**
- Esは太陽活動より大気の風のシアに依存するため、外部指標からの短時間予測は研究者でも未解決
- 4局FxEsデータのみでは時間・空間の事前予測は原理的に限界がある

**今後の方針：「今出ているを正確・直感的に見せる」に集中**
1. 「今出ている」をいち早く知る → LINE通知（すでに実現済み）
2. 「今出ているとき、どこ向けが開いているか」を直感的に見せる → 地図表示の磨き込み

### 現状
- es-demo.htmlでライブテスト運用中
- karamasu0134がフィードバックを収集中
- 気になる点があれば随時改善予定

---

## 2026-05-27 — Discord セッション（es-demo プロジェクト終了）

### 議論のまとめ・結論

**4局イオノゾンデデータの限界を明確化**
- 各局の真上のEsの強さ（点データ）しか分からない
- 地点間の状況・移動方向・事前予測は原理的に不可能
- 東西移動の検出も不可（4局がほぼ同一経度に並ぶため）
- 「南北のシフト傾向」は読めるが「移動方向」とは呼べない粗さ

**GNSS-TECによる面的観測について**
- GEONET（国土地理院）の生データは公開されているが処理が気象機関レベルの難易度
- 個人プロジェクトでのリアルタイム処理は無理と判断
- NICTが保有する研究用データも一般公開APIなし

### プロジェクト終了
es-demoを用いた予測機能の開発探求を終了。現行機能（地図表示）の範囲でできることはやり切った。

**⚠️ LINE通知 停止（2026-05-27）**
- 無料利用枠超過のため LINE Messaging API 通知を停止
- 現在はブラウザ側のアラートバナーがメインのアラート手段
- Worker の Cron 自体は残存（CORSプロキシとして継続稼働）

**継続稼働中のもの**
- Cloudflare Worker: CORSプロキシとして稼働（LINEアラートは停止）
- docs/index.html: メインWebアプリ（ブラウザ直接取得）
- docs/es-demo.html: テストページとして残置（本番には組み込まない）

---

## 2026-06-05 — Discord セッション

### FxEs 閾値修正（monitor.html / worker/index.js）
- FxEs 黄色閾値・Web Push 通知閾値・通知メッセージを 7.0 → **6.0 MHz** に変更（前回セッションより）

### LINE 通知の完全停止
- worker/index.js の Cron LINE送信ループを削除
- コメント残置：`// LINE通知は2026-06-02 完全停止（無料枠超過のため）`

### Es 影響圏マップ新規作成（`docs/es-coverage.html`）
- **企画背景**：foEs/FxEs 値から「各観測局を中心としたEs影響圏」を視覚化する新ページ
- **設計方針**：
  - QTH依存なし（各局の状況を表示、ユーザーが自分の位置で判断）
  - foEs = FxEs − 0.6 MHz で換算（NICTのリアルタイムは FxEs のみ公開）
  - 真円の半透明グラデーションで影響圏を表現
- **閾値（foEs → 円半径）**：
  - 5〜8 MHz: 青（〜200km）
  - 8〜10 MHz: 黄橙（〜700km）
  - 10〜12 MHz: 橙（〜900km）
  - ≥ 12 MHz: 赤（〜1200km）
- data.json から FxEs をリアルタイム取得（5分ごと自動更新）

### GitHub Actions 停止問題の対策
- **症状**：本日 10:54 以降 Fetch Iono Data が停止（約4時間更新なし）
- **原因（推定）**：git push 競合で連続失敗 or GitHub スケジューラが一時停止
- **対策①**：fetch-data.yml の push ステップに `pull --rebase` リトライを追加
- **対策②**：`keepalive.yml` 新規作成（毎週日曜 GitHub API で fetch-data.yml を再有効化）

### es-coverage.html 追加調整（同日セッション後半）
- データ取得を GitHub Pages 静的 data.json → Cloudflare Worker 経由 NICT 直接取得に変更（monitor.html と同方式）
- firstChild→children[0] バグ修正（「読み込み中」で止まる問題）
- 4局の SVG 点位置をユーザー指示で調整（沖縄・鹿児島・東京・北海道）
- 沖縄の最大影響圏を 100px（約523km相当）に上限設定（本州との視覚的重なりを抑制）
- 手動更新ボタン追加（↻ 更新）

---
<!-- 以下に新しいセッションの記録を追加 -->

## 2026-04-14 15:44 — Auto-log
49d78ee Remove browser-side LINE sending, LINE is now handled by Cron Worker
34e1447 data: update 06:17
33b6a4e data: update 04:20
1311885 data: update 01:08
1a0b27f data: update 23:29

---

## 2026-04-26 — Desktop セッション

- `docs/guide.html` 新規作成
  - Es伝播の仕組み（SVGイラスト、ワンスパン距離、λ/4アンテナ特性）
  - FxEsカラースケール説明
  - MUFカード（F2層伝搬であることを明記、高度比較SVG）
  - アンテナ傾きテクニック（傾け角テーブル、チップ変位量、スマホアプリによる計測）
  - Esリフレクションポイントシミュレーター（47都道府県ドロップダウン、スキップゾーン可視化）
- `docs/privacy.html` 新規作成
  - 運営者情報・LINE UserID取り扱い・NICT/NOAAデータ利用条件・免責事項
- `docs/index.html` 更新：データ出典に利用条件追記、フッターにプライバシーポリシーリンク追加
- `docs/monitor.html` 更新：LINE通知モーダルに注記追加、フッターにリンク追加

### LINE通知不達バグ修正・仕様変更（重要）
- **症状**：2026-04-28 07:05 JST、東京国分寺 FxEs=9.3 MHz 検出 → LINE通知届かず
- **原因**：`worker/index.js` で `pushLine()` の戻り値を確認せずクールダウンをセット
  → push失敗でもクールダウンが2時間有効になり、次回クーロンでの再試行がブロックされていた
- **即時対処**：本番KVの全受信者の "to" クールダウンをクリア（手動、wrangler経由）
- **コード修正**：`pushLine()` の戻り値（`res.ok`）を `sent` で受け取り、`true` の場合のみクールダウンセット
- **デプロイ**：`npx wrangler deploy` 完了（Version ID: 860ba167）
- **根本原因 2**：LINE無料プランの月間上限200通を使い切っていた（9人×複数回）
  - `/line-status` エンドポイント追加で即時確認可能に
  - pushLine失敗時にHTTPステータス・レスポンスボディをログ出力するよう改善
- **仕様変更**：システム全体の通知時間帯を 05:00〜20:00 JST に統一（Version ID: 7a5f17e3）
  - 夜間の不要な送信をなくし月間消費を削減
  - サイト（index.html・monitor.html）にお知らせバナーを追加（5/6まで表示）
- **注意**：5月1日に月間通数リセット → 通知復旧予定

---

## 2026-05-02 — Desktop セッション

### Web Push通知の実装（LINE通知から移行）
- **背景**：Es盛期に登録者14名×複数回でLINE月200通上限に即日到達
- **方針**：LINE通知は3名（管理者・LV206・せたがやHY19）のみ継続、他はWeb Pushに移行

#### Workerの変更（`worker/index.js`）
- Web Push送信実装（RFC 8291 aes128gcm暗号化 + RFC 8292 VAPID JWT）
  - CF Workers ネイティブ Web Crypto API で実装（外部ライブラリ不使用）
  - ECDH共有シークレット → HKDF → AES-128-GCM
- VAPIDキーペア生成・`VAPID_PRIVATE_JWK` をWorkerシークレットに登録
- `?action=subscribe` (POST) / `?action=unsubscribe` (DELETE) エンドポイント追加
- `checkAndAlert()` にLINEホワイトリスト（3名）チェックを追加
- Web Push用クールダウン（`webpush_state` KVキー）を追加
- **デプロイ**：Version ID `1b11f008`

#### フロントエンドの変更
- `docs/sw.js` 新規作成（Service Worker：push受信・通知クリック）
- `docs/index.html`：「Es発生通知を登録する」ボタン追加、iPhone向け手順モーダル追加
- `docs/monitor.html`：LINE申込ボタン → 通知登録ボタンに変更、モーダル追加

#### LINE終了アナウンス
- 11名（ホワイトリスト外）に終了アナウンスをLINE送信（全員 ok: true）
- 送信時点の消費数: ~191/200通

---

## 2026-05-02 続き — Desktop セッション

### PWAアイコン整備
- ホーム画面アイコンが「C」になる問題を修正
- `docs/icon-192.png` / `docs/icon-512.png` 新規作成（Python stdlib で生成、navy背景＋amber「Es」文字）
- `docs/manifest.json` を `index.html` / `monitor.html` の `<head>` に `<link rel="manifest">` で紐付け
- `apple-mobile-web-app-capable` / `apple-mobile-web-app-title` / `apple-touch-icon` メタタグ追加
- ユーザー（karamasu0134）がiPhone PWAでの通知登録を完了確認済み

### iPhone通知登録ガイドカードの追加
- `docs/index.html`：ボタンのサブテキストから「無料・無制限」を削除
- メニューカード下にiPhone向け通知登録ガイドカードを新設
  - 「iPhoneはホーム画面への追加が必要」の事前説明
  - 実機スクショ付き4ステップ手順（…→共有、ホーム画面に追加）
- `docs/ios-step1.png` / `docs/ios-step2.png`：実機スクショをPython stdlibで300px幅にクロップ・リサイズして追加

### LINE移行アナウンスバナーの追加
- `docs/index.html`：announce-banner div を復活・内容をLINE→Web Push移行のお知らせに変更
  - その日の初回閲覧時に表示、✕で当日非表示
  - KEY: announce_20260502、期限: 2026-05-08T15:00:00Z（5/8 JST末まで）
  - 旧バナー（announce_20260428）から差し替え

### LINE新規登録ブロック・非WL者への終了通知（重要）
- **経緯**：LINE終了アナウンス後に新規登録者が発生
- **`worker/index.js` 修正**：
  - `handleFollow`：登録フローを停止、友達追加時にWeb Push誘導メッセージを自動返信
  - `handleMessage`：ホワイトリスト外ユーザーのメッセージに終了案内を返す
  - `?action=farewell-new`（GET）：非WL登録者への一斉終了通知エンドポイント（使用後は残置、再実行可）
- **一斉送信実施**：非WL登録者12名に送信 → ok:true 5名、ok:false 7名（ブロック/退会済み）
- **デプロイ**：Version ID `961612a2`

---

## 2026-05-08 — Discord セッション

### FxEs履歴蓄積を追加
- `docs/fxes-history.json` 新規作成：git履歴448件（4/9〜現在）からFxEsデータを抽出・保存
  - 形式：`[{ts, ok, yg, to, wk}, ...]`（UTCタイムスタンプ、昇順）
  - ファイルサイズ：約49KB
- `scripts/fetch-data.js` 改修：15分更新ごとにfxes-history.jsonへ追記
  - 重複タイムスタンプはスキップ
- `scripts/extract-fxes-history.js` 追加：初回抽出スクリプト（実行済み、以後不要）
- OPERATION LOGにCB×Es分析カードを追加（ログ/検索タブ末尾）
  - 年月セレクター → その月のCB交信一覧 × FxEs4地点
  - FxEs 9.0以上を赤字強調
  - 「テスト中」バッジ付き

### NICTアーカイブデータ取り込み（2025-2026全局）
- NICTアーカイブから4局×2年分を取り込み → `docs/fxes-history.json` を大幅拡充
  - 取り込み前：448件（4/9〜現在、gitのみ）
  - 取り込み後：**70,080件**（2025/1/1〜2026/12/31）
  - 対象局：OK426（沖縄）・YG431（鹿児島）・TO536（東京）・WK546（北海道）
  - ファイルサイズ：5.0MB（コンパクトJSON形式）
  - データソース：https://wdc.nict.go.jp/Ionosphere/archive/observation-history/
- `scripts/merge-nict-history.js`：NICTアーカイブマージスクリプトを追加
- `scripts/fetch-data.js`：fxes-history.json書き込みをコンパクト形式に変更
- 2025年7月のEsイベント（9MHz超）1,294件を収録
- 昨年5月からのQSO履歴とFxEsの突き合わせが可能になった

---

## 2026-05-19 — Discord セッション

### fxes-history.json 自動更新バグ修正
- **症状**：5/18以降のFxEsデータがCB×Es分析に表示されない
- **原因①**：`.github/workflows/fetch-data.yml` の `git add` に `fxes-history.json` が含まれていなかった
- **原因②**：2026アーカイブ取り込み時に12月末まで全て`--`の空行が入り込み、fetch-data.jsが「重複タイムスタンプ」としてスキップ
- **修正①**：fetch-data.yml に `docs/fxes-history.json` を追加（根本修正）
- **修正②**：fetch-data.js: 重複タイムスタンプでも値を上書きするよう変更
- **補完**：2026アーカイブ最新版で5/9〜5/18の欠落データを補完（47,560件）
- 次回GitHub Actions実行から5/19以降も自動蓄積される

---

## 2026-05-23 — Discord セッション

### GitHub Actionsの失敗メール対応
- **症状**：5/19以降、15分ジョブが高頻度で失敗しメール通知が届くようになった
- **原因**：5/19の修正でfxes-history.jsonを15分ジョブに追加したが、複数ジョブ同時実行で毎回push競合が発生
- **修正**：fxes-history.jsonの更新を日次専用ジョブに分離
  - `fetch-data.yml`：fxes-history.jsonのgit addを削除（data.jsonのみ）
  - `update-fxes-history.yml`：新規作成（毎日01:30 JST / 16:30 UTC）
    - NICTアーカイブ（前年・当年の4局分）を再ダウンロード
    - `merge-nict-history.js` を実行してfxes-history.jsonを更新・コミット
  - `merge-nict-history.js`：年自動検出・全--除外・NICTデータで上書き対応

---

## 2026-06-09 12:50 — Discordセッション

- `docs/es-coverage.html` をD3.js地理的地図に全面書き換え
  - D3.js v7 + topojson-client@3 + world-atlas countries-50m.json
  - equirectangular投影、日本+台湾（ID:392/158）
  - 4観測点ドット・ラベル表示（稚内/国分寺/山川/大宜味）
- 投影パラメータ調整（多数試行）
  - fitSize → 手動scale → angle()+fitSize → fitExtent負パディング と段階的改善
  - 最終: angle(+20°) + fitExtent(pad=-30) + translate-40px上シフト
  - SVGをviewBox対応レスポンシブ化（iPhone 375px画面幅対応）
- 地図レイアウト確定（稚内右上・台湾左下の対角配置）。以降変更しないこと
- 次の作業：Es影響圏の円描画・FxEsデータ取得・表示機能の実装（指示待ち）

---

## 2026-07-26 02:05 — Discordセッション

- 通知しきい値を6.0→7.0に戻す（誤って6.0に変更されていたため）
  - `worker/index.js`：判定条件（`v >= 6.0` → `v >= 7.0`）、通知メッセージ本文の2箇所を修正
  - `npx wrangler deploy` でデプロイ済み
